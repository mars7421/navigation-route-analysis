...
if page == "홈":
    ...
    **경로 유형 요약**  
    ...
    st.markdown("""
    - 실제 교통 데이터 활용한 예측 모델 아님
    - 도로 구조 영향 설명 목적의 시뮬레이션 모델 
    """)
    ...
    👉 정밀한 분석을 위해 다변량 기반 KPI를 활용한 ETA 모델링 및 경로 평가 로직 설계 필요
    """)
    ...
    - Turns: 높은 상관 (다른 구조 영향 포함) ⇒ 독집적 영향 X  
    - Density: 독립적 영향은 제한적  
    """)
    ...
    st.header("분석 인사이트")
    st.markdown("""
    ETA가 단일 요소가 아닌, **여러 도로 구조 요소가 결합된 결과**임을 보여줌  
    또한 도로 구조 KPI를 통해 단순 시간 비교를 넘어 **경로별 주행 특성을 해석할 수 있음**  
